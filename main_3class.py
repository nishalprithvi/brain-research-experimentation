...
def main():
    ...
    ft_parser.add_argument("--use_balanced_sampler", action="store_true")
    ft_parser.add_argument("--sampler_power", type=float, default=1.0)
    ft_parser.add_argument("--loss_mode", type=str, default="ce", choices=["ce", "balanced_softmax", "logit_adjusted_ce"])
    ft_parser.add_argument("--logit_adjust_tau", type=float, default=1.0)
    ft_parser.add_argument("--auc_guardrail", type=float, default=0.7253)
    ft_parser.add_argument("--synthetic_selection_mode", type=str, default="random", choices=["random", "edge_strength_topk"])
    ft_parser.add_argument("--minority_tail_epochs", type=int, default=0)
    ft_parser.add_argument("--minority_tail_lr_scale", type=float, default=0.5)
    ft_parser.add_argument("--minority_tail_use_balanced_sampler", action="store_true")
    ft_parser.add_argument("--model_select_mode", type=str, default="macro_guard", choices=["macro_guard", "phase1_target"])
    ft_parser.add_argument("--target_acc", type=float, default=0.74)
    ft_parser.add_argument("--target_macro_f1", type=float, default=0.58)
    ft_parser.add_argument("--target_auc", type=float, default=0.81)
    ft_parser.add_argument("--target_ad_recall", type=float, default=0.70)
    ft_parser.add_argument("--target_mci_recall", type=float, default=0.40)
    ...
    if args.command == "train":
        ...
        sys.argv = [
            ...
            str(args.scale_mci),
            "--teacher_model_type",
            str(args.teacher_model_type),
            "--n_ad_override",
            str(args.n_ad_override),
            "--n_mci_override",
            str(args.n_mci_override),
            "--quality_track_samples",
            str(args.quality_track_samples),
            "--quality_track_stride",
            str(args.quality_track_stride),
            "--quality_conf_threshold_ad",
            str(args.quality_conf_threshold_ad),
            "--quality_conf_threshold_mci",
            str(args.quality_conf_threshold_mci),
            "--quality_spectral_topk",
            str(args.quality_spectral_topk),
            "--quality_spectral_tau",
            str(args.quality_spectral_tau),
            "--quality_dup_real_th",
            str(args.quality_dup_real_th),
            "--quality_dup_intra_th",
            str(args.quality_dup_intra_th),
            "--quality_max_samples",
            str(args.quality_max_samples),
            "--quality_edge_threshold",
            str(args.quality_edge_threshold),
        ]
