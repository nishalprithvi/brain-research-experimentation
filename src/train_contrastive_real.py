...
def train_contrastive(epochs=100, batch_size=32):
            ...
            if weights is not None:
                ...
                if k > 0:
                    ...
                    if len(remove_indices) > 0:
                        ...
                        # print(f"Sparsified: {original_edges} -> {g.num_edges()} edges")
                        
        return glist
        
    glist_real = sanitize(glist_real)
    
    # NO Synthetic Data Loading
    
    full_dataset = glist_real
    print(f"Pre-Training on {len(full_dataset)} graphs (Real Data Only).")
    ...
    # 3. Training Loop
    print("\n--- Starting Contrastive Pre-Training (Real Data Only) ---")
    ...
    save_path = os.path.join(save_dir, 'gcn_pretrained_contrastive_real.pth')
